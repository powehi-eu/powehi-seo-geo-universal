...
def _runtime_extension_scripts(text: str) -> set[str]:
    return set(
        re.findall(r"powehi-seo-geo run --extension banana ([A-Za-z0-9_]+\.py)", text)
    )
...
def test_core_image_skill_does_not_assume_banana_scripts_are_local():
    ...
    assert "~/.claude/skills/powehi-seo-image-gen/scripts/" not in combined
...
def test_standalone_installer_copies_scripts_beside_skill_file():
    ...
    assert '"$HOME/.claude/skills/powehi-seo/bin/powehi-seo-geo" run' in installer
